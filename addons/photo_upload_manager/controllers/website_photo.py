...
class WebsitePhoto(http.Controller):
    ...
    def upload_photo(self, **kwargs):
        """Controller per caricare l'immagine, salvare il record in photo.order e aggiungere il prodotto al carrello."""
        try:
            ...
            if not order_id or not product_id or not uploaded_file:
                _logger.warning("Parametri mancanti: order_id=%s, product_id=%s, image=%s", order_id, product_id, uploaded_file)
            ...
            # Crea il record in photo.order, impostando anche name = order.name
